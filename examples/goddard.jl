...
#
# tf is free, so the mesh is given nominal placements and `unknown_horizon = true` puts a horizon
# variable `tscale` in the model. Every residual then reads h[i] and t[i,k] off it, and tscale is tf.
...
# Thrust is bang-singular-bang, so the control chatters along the singular arc on any fixed mesh
function examodel_goddard(; tf0 = 0.2, N = 60, K = 3)
    # The nominal span is the start value for tscale
    core = CollocationExaCore(range(0.0, tf0; length = N + 1), K; unknown_horizon = true)

    # Bounds and a start guess, per state: h >= h0, v >= 0, mf <= m <= m0
