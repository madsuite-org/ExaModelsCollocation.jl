...
@testset "Bruno et al. (2016) parameter estimation" begin
    ...
    begin
        ...
        KP = [7, 8, 9, 10, 11, 12]   # rate cvidx -> its parameter index
        SZEA = 13
    ...
    # ----- Creating mesh nodes -----
    begin
        # Refine mesh by capping interval widths by hmax
    ...
    function build(θ0 = θnom)
        ...
        mesh_t = core.mesh.t
        sweep(rows) = [(first(row),c,Base.tail(row)...,i,k,mesh_t[i,k])
                       for row in rows, c in 1:Nc, i in 1:N, k in 1:K]

        # Create collocation constraints
        # NOTE: seven species, four distinct expressions, so four calls over the one block
        # bcar, zea: one sink
        @add_con_collocation(core, coll_decay1, z,
            -cv[cvidx,c] * z[v,c,i,k]
            for (v,c,cvidx,i,k,t) in sweep([(BCAR, KB1), (ZEA, K5)])
        )

        # bcry: two sinks
        @add_con_collocation(core, coll_decay2, z,
            -(cv[cvidx1,c] + cv[cvidx2,c]) * z[v,c,i,k]
            for (v,c,cvidx1,cvidx2,i,k,t) in sweep([(BCRY, KC1, KC2)])
        )

        # b10, ohb10: two sources and a sink
        @add_con_collocation(core, coll_flow, z,
            cv[cvidx1,c] * z[v1,c,i,k] + cv[cvidx2,c] * z[v2,c,i,k] - cv[cvidx3,c] * z[v,c,i,k]
            for (v,c,cvidx1,v1,cvidx2,v2,cvidx3,i,k,t) in sweep([
        ...
        @add_con_collocation(core, coll_sink, z,
            cv[cvidx1,c] * z[v1,c,i,k] + cv[cvidx2,c] * z[v2,c,i,k] + cv[cvidx3,c] * z[v3,c,i,k]
            for (v,c,cvidx1,v1,cvidx2,v2,cvidx3,v3,i,k,t) in sweep([
    ...
    begin
        ...
        @testset "from the nominal parameters" begin
            ...
            @test only(core.blocks).dims == (1:Nz, 1:Nc)
            @test core.nvar == Nz * Nc * N * (K + 1) + Np + Ncv * Nc
            @test Nm == 77

            result = madnlp(ExaModels.ExaModel(core); print_level = MadNLP.ERROR)
            @test result.status == MadNLP.SOLVE_SUCCEEDED

            # K = 4 Radau on this mesh has to reproduce the likelihood an adaptive ODE solver
            # gets on the same parameters
            ...
            # Concentrations stay nonnegative and the species only ever consumed decay
            # monotonically; neither is imposed, so both are the discretization's own
            @test all(zsol .>= -1e-6)
            ...
            cvsol = ExaModels.solution(result, cv)
            @test cvsol[KB1, 1] ≈ 0.0 atol = 1e-8
            @test cvsol[KB1, 3] ≈ 10.0^θ[KP[KB1]] * 10.0^θ[SZEA] rtol = 1e-6
        ...
        @testset "from a perturbed start" begin
            # The starting point is moved off the optimum, so the guess is doing real work
